...
void JointGimServo::update() {
  ...
  if(!is_torque_on) {
    ...
    uint16_t           u16_tgt_pos = (uint16_t)(FL_ANG_DEG_TO_RAW * fl_raw_tgt_deg + 32768);

    /* データ作成 */
    p_txparams->u8_pos_h = u16_tgt_pos >> 8;
    p_txparams->u8_pos_l = u16_tgt_pos & 0x00FF;
    p_txparams->u8_vel_h = 0;
    p_txparams->b4_vel_l = 0;
    p_txparams->b4_Kp_h  = u16_Kp >> 8;
    p_txparams->u8_Kp_l  = u16_Kp & 0x00FF;
    p_txparams->u8_Kd_h  = u16_Kd >> 4;
    p_txparams->b4_Kd_l  = u16_Kd & 0x000F;
    p_txparams->b4_trq_h = 0;
...
void JointGimServo::rx_callback(GimMsgRx *rxMsg, int16_t microsec_id) {
  ...
  /* 電流情報の保存 */
  int16_t s16_rx_curr_raw = (int16_t)(rxMsg->b4_trq_h << 8 | rxMsg->u8_trq_l) - 2048;
